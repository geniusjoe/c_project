...
int find(int u, int v, int type, int k) { /**< type==0 代表最大值， type==1 代表求和 */
    ...
    while(f1 != f2) {
        ...
        if(type == 0)
            tmp = max(tmp, query(0, pos - 1, p[f1], p[u], T[raw_city[k][1]], type));
        else
            tmp += query(0, pos - 1, p[f1], p[u], T[raw_city[k][1]], type);
        u = fa[f1];
        f1 = top[u];
    }
    //if(u == v)
    ...
    if(deep[u] > deep[v])
        swap(u, v);
    if(type == 0)
        return max(tmp, query(0, pos - 1, p[u], p[v], T[raw_city[k][1]], type));
    else
        return tmp + query(0, pos - 1, p[u], p[v], T[raw_city[k][1]], type);
}
int e[MAXN];
...
void work1() {
    for(int i = 1; i <= event_num; i++) {
        ...
        if(tmp1 == 'Q' && tmp2 == 'S') {
            ...
            T[raw_city[city1][1]] = update(0, pos - 1, T[raw_city[city1][1]], p[city1], -raw_city[city1][0]);
            raw_city[city1][1] = city2;
            T[raw_city[city1][1]] = update(0, pos - 1, T[raw_city[city1][1]], p[city1], raw_city[city1][0]);
            continue;
        } else if(tmp1 == 'C' && tmp2 == 'W') {
            int tmp = raw_city[city1][0];
            raw_city[city1][0] = city2;
            T[raw_city[city1][1]] = update(0, pos - 1, T[raw_city[city1][1]], p[city1], -tmp + raw_city[city1][0]);
            continue;
        }
    }
